...
public final class PixelImage extends BufferedImage
{
  ...
  /**
   * Constructs a new PixelImage with the specified dimensions and type.
   * This constructor is not to be called externally.
   * 
   * @param the_width The width.
   * @param the_height The height.
   * @param the_type The type.
   * @see BufferedImage(int, int, int)
   */
  private PixelImage(final int the_width, final int the_height, 
                     final int the_type)
  {
   ...
   */
  public static PixelImage load(final File the_file) 
    throws IOException
  {
    ...
    final PixelImage image = 
      new PixelImage(buf.getWidth(), buf.getHeight(), 
   ...
   */
  public void save(final File the_file) 
    throws IOException
  {
  ...
  /**
   * Return the image's pixel data as an array of Pixels. The first
   * coordinate is the y-coordinate, so the size of the array is
   * [height][width], where width and height are the dimensions of the
   * image.
   * 
  ...
  public Pixel[][] getPixelData()
  {
    ...
    int[] samples = new int[Pixel.CHANNELS];
    ...
    for (int row = 0; row < r.getHeight(); row++)
    {
      for (int col = 0; col < r.getWidth(); col++)
      {
        ...
        final Pixel new_pixel = 
          new Pixel(samples[0], samples[1], samples[2]);
   ...
   * match the image's dimensions or that has pixels with invalid values
   * (not 0-255).
   * 
   * @param the_data The pixel data.
   * @exception IllegalArgumentException if the pixel data does not match
   * the image dimensions or has invalid pixel values.
   */
  public void setPixelData(final Pixel[][] the_data)
    throws IllegalArgumentException
  {
    final int[] pixel_values = new int[Pixel.CHANNELS];
