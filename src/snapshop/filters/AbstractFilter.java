...
{
  /**
   * The suffix to remove from filter class names.
   */
   ...
   * description.
   */
  //@ ensures getName().equals(getClass().getName());
   ...
   * @param the_weights The weights matrix. This must be a non-null 3x3 
   * matrix or an IllegalArgumentException is thrown.
   * @exception IllegalArgumentException if the weights are invalid.
   */

  protected void weight(final PixelImage the_image, 
                        final int[][] the_weights)
  ...
  protected void weight(final PixelImage the_image, 
                        final int[][] the_weights, final int the_scale)
    throws IllegalArgumentException
  {
    checkWeights(the_weights);
   ...
   * non-null and a 3x3 grid).
   * 
   * @param the_weights The weights matrix.
   * @exception IllegalArgumentException if the weights matrix is invalid.
   */

  private void checkWeights(final int[][] the_weights) 
    throws IllegalArgumentException
  {
    if (the_weights == null || the_weights.length != Pixel.CHANNELS || 
        the_weights[0] == null || the_weights[0].length != Pixel.CHANNELS || 
        the_weights[1] == null || the_weights[1].length != Pixel.CHANNELS ||
        the_weights[2] == null || the_weights[2].length != Pixel.CHANNELS)
    {
      throw new IllegalArgumentException("must pass 3x3 weights grid");
    }
  }
  ...
  {
    return Math.max(Pixel.MIN_COLOR, Math.min(Pixel.MAX_COLOR, the_color));
  }
  ...
  protected void swap(final Pixel[][] the_data, final int row_1, 
                      final int col_1, final int row_2,
                      final int col_2)
  {
